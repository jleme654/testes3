...
public class Solution4 {
	...
	public static void main(String[] args) {
		...
		int linhaPessoas = sc.nextInt(), colunaTopicos = sc.nextInt(), num = 0, countSabem = 0,
				maior = -1, numMelhoresTimes = 0;
		...
		for (int i = 0; i < linhaPessoas - 1; i++) {
			for (int j = i + 1; j < linhaPessoas; j++) {
				...
				if (countSabem > maior) {
					maior = countSabem;
					numMelhoresTimes = 1;
				}else if(countSabem == maior){
					numMelhoresTimes ++;
				}
			}
		}
		System.out.println(countSabem	);
		System.out.println(numMelhoresTimes);
	}
